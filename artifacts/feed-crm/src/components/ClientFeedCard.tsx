...
import {
  ...
  XCircle, Trash2, Sparkles, RefreshCw, Zap, TrendingUp, AlertTriangle,
  MessageSquarePlus, History, ChevronRight
} from "lucide-react";
...
export function ClientFeedCard({
  ...
  return (
    <div className="w-full h-[var(--feed-card-height)] flex-shrink-0 snap-start snap-always flex flex-col p-4">
      <Card className={`relative w-full h-full flex flex-col overflow-hidden transition-all duration-300 ${isActive ? 'scale-100 opacity-100 shadow-2xl shadow-primary/10 border-primary/30' : 'scale-[0.98] opacity-70'} bg-card border-card-border`}>
        ...
        <div className="relative flex justify-between items-start p-4 pb-2 shrink-0">
          <div className="flex flex-col gap-0.5 min-w-0">
            <h2 className="text-xl font-bold text-foreground leading-tight tracking-tight truncate">{client.name}</h2>
            <p className="text-muted-foreground text-xs font-medium truncate">{client.product || "Sem produto"}</p>
          </div>
          <div className="flex gap-2 items-center shrink-0">
            <Badge variant="outline" className={`capitalize font-bold border rounded-full px-2 py-0.5 text-[11px] ${getTemperatureColor(client.temperature)}`}>
              {client.temperature}
            </Badge>
            <DropdownMenu>
        ...
        <div className="relative px-4 py-1.5 flex items-center justify-between gap-3 shrink-0">
          <div className="flex flex-col flex-1 gap-1">
            <div className="flex justify-between text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              <span className="flex items-center gap-1"><TrendingUp className="h-3 w-3" /> Chance</span>
              <span className="text-primary">{client.chancePurchase}%</span>
            </div>
            <Progress value={client.chancePurchase} className="h-1 bg-primary/20" indicatorClassName="bg-primary" />
          </div>
          <div className="flex flex-col flex-1 gap-1">
            <div className="flex justify-between text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              <span className="flex items-center gap-1"><AlertTriangle className="h-3 w-3" /> Risco</span>
              <span className="text-destructive">{client.riskLoss}%</span>
            </div>
            <Progress value={client.riskLoss} className="h-1 bg-destructive/20" indicatorClassName="bg-destructive" />
          </div>
        </div>

        {/* Status Indicators */}
        <div className="relative flex gap-1.5 px-4 py-2 flex-wrap shrink-0">
          <Badge variant="outline" className={`text-[11px] font-semibold rounded-md border ${getTimeAgoColor(client.lastInteractionAt)}`}>
            {formatTimeAgo(client.lastInteractionAt)}
          </Badge>
          <Badge variant="outline" className={`text-[11px] capitalize font-semibold rounded-md border ${getPriorityColor(client.priority)}`}>
            {client.priority}
          </Badge>
          {client.stage && (
            <Badge variant="secondary" className="text-[11px] font-semibold rounded-md bg-secondary text-secondary-foreground">
              {client.stage}
            </Badge>
          )}
        </div>
        ...
        <div className="relative flex-1 px-4 py-1 flex flex-col gap-2 min-h-0 overflow-hidden">
          {client.mainObjection && (
            <div className="bg-destructive/5 border border-destructive/10 rounded-lg px-3 py-2 shrink-0">
              ...
              <p className="text-xs font-medium text-foreground/90 truncate">{client.mainObjection}</p>
            </div>
          )}
          ...
          <div className="bg-primary/5 border border-primary/10 rounded-lg p-3 flex-1 flex flex-col min-h-0">
            <div className="flex justify-between items-center mb-1.5 shrink-0">
              <p className="text-[10px] font-bold uppercase tracking-wider text-primary flex items-center gap-1">
                <Sparkles className="h-3.5 w-3.5" /> Ação · Assistente Inteligente
              </p>
              <Button
                variant="ghost"
                size="sm"
              ...
              >
                ...
                {regenerateSuggestion.isPending ? "Gerando..." : "Gerar sugestão"}
              </Button>
            </div>
            ...
            {client.suggestedMessage && (
              <div className="mt-auto bg-card rounded-md p-2.5 border border-border">
                <p className="text-[10px] font-medium text-muted-foreground mb-0.5">Sugestão de mensagem:</p>
                <p className="text-xs text-foreground/80 italic line-clamp-2">"{client.suggestedMessage}"</p>
                <button
                  type="button"
